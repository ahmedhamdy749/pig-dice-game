(function(){
    ...
    'use strict';
    const startGame = document.getElementById('stratgame');
    ...
    startgame.addEventListener("click", function () {
        ...
        gameControl.innerHTML += '<button id = "quit"> do you want quite</button>';
    ...
    function throwDice(){
        ...
        if ( gameData.rollSum === 2){
            ...
            gameData.index ? (gameData.index = 0) : (gameData.index = 1);
            game.innerHTML+= `<p>you rolled a one , 
                swiching to ${gameData.players[gameData.index]} </p>`;
            setTimeout(setUPTurn,2000);    
        }
        else{
    ...
    function showCurrentScore(){
        score.innerHTML = `<p>The score is currently <strong>
                ${ gameData.players[0]} ${gameData.score[0]}
                </strong> and <strong> ${gameData.players[1]}
